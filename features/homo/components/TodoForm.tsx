...
export default function TodoForm({
  ...
  async function onSubmit(values: FormValues) {
    try {
      const images =
        values.imageUrls?.filter((u): u is string => typeof u === 'string' && u.length > 0) ??
        [];
      const legacyImageUrl = images.length > 0 ? images[0] : undefined;
      // Backward-compat: some deployments may only support `imageUrl`.
      // Enable `imageUrls` explicitly once Convex prod schema/mutations are deployed.
      const enableMultiImages =
        process.env.NEXT_PUBLIC_ENABLE_TODO_IMAGEURLS === 'true';
      ...
      if (mode === 'create') {
        ...
        await create({
          ...
          imageUrls: enableMultiImages && images.length > 0 ? images : undefined,
        });
