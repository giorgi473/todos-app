...
import { Calendar } from '@/components/ui/calendar';
import { CalendarIcon, Loader2, Sparkles, Plus, X } from 'lucide-react';
import { formSchema, type FormValues } from '@/features/homo/schema/formSchema';
import { priorityConfig } from '@/features/homo/lib/priority-config';
...
export default function TodoForm({
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      priority: initialData?.priority ?? 'medium',
      dueDate: initialData?.dueDate ? new Date(initialData.dueDate) : undefined,
      imageUrls: [], // ივსება ქვემოთ useEffect-ით
    },
  });

  // sync form.imageUrls with imagePreviews
  useEffect(() => {
    form.setValue('imageUrls', imagePreviews);
  }, [imagePreviews, form]);
